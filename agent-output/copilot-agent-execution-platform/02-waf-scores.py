"""Generate 02-waf-scores.png — WAF pillar scores chart."""
import os
...
def generate_waf_chart(scores: dict, output_path: str) -> None:
    ...
    for bar, val in zip(bars, values):
        ...
        color_ = "white" if val >= 1.5 else "#333"
        ax.text(label_x, bar.get_y() + bar.get_height() / 2,
                f"{val:.1f}", va="center", ha="right" if val >= 1.5 else "left",
                fontsize=11, fontweight="bold", color=color_)
...
if __name__ == "__main__":
    ...
    generate_waf_chart(scores, os.path.join(here, "02-waf-scores.png"))
