"""Generate 03-des-cost-projection.png — 6-month cost projection with trend + budget cap."""
import os
...
def generate_cost_projection_chart(months, costs, output_path, budget_cap=None, growth_assumption=None):
    ...
    ax.set_ylim(0, max(max(costs), budget_cap or 0) * 1.15)
...
if __name__ == "__main__":
    # Projection: baseline $1,971 ramping with sandbox autoscale + LA growth
    months = ["Month 1", "Month 2", "Month 3", "Month 4", "Month 5", "Month 6"]
    costs  = [1971, 2120, 2300, 2510, 2750, 3020]
    here = os.path.dirname(os.path.abspath(__file__))
    generate_cost_projection_chart(
        months, costs,
        output_path=os.path.join(here, "03-des-cost-projection.png"),
        budget_cap=5000,
        growth_assumption="Assumes ~10% MoM sandbox concurrency growth + linear LA ingestion growth",
    )
